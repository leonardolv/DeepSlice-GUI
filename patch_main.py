...
# 5. Fix setTexts for session label
# we need to replace self.session_status_label.setText(X) with:
# self._session_base_text = X
# self._update_session_status()
content = re.sub(
    r"self\.session_status_label\.setText\(\s*([^\)]+?)\s*\)",
    r"self._session_base_text = \1\n        self._update_session_status()",
    content
)
